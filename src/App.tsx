import { ArrowRight, CheckCircle2, Package, TrendingUp, Truck, UserPlus } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#0F172A]">
      ...
      <header className="sticky top-0 z-30 border-b bg-white/95 backdrop-blur">
        <div className="mx-auto w-full max-w-[1100px] px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          <nav className="flex items-center gap-3 sm:gap-5">
            ...
            <a href={`${CONNECT}/analyzer`} className="hidden sm:inline-flex items-center text-sm font-medium text-slate-600 hover:text-[#2A3CDE] hover:underline transition-colors">
              Import Bulk Analyzer
            </a>
            <a href={`${CONNECT}/register`} className="hidden sm:inline-flex items-center gap-1.5 rounded-lg border border-[#2A3CDE] px-4 py-2 text-sm font-semibold text-[#2A3CDE] hover:bg-blue-50 transition-colors whitespace-nowrap">
              <UserPlus className="h-4 w-4" /> Apply as Partner
            </a>
            <a href={`${CONNECT}/login`} className="inline-flex items-center rounded-lg bg-[#2A3CDE] hover:bg-[#2A3CDE] px-4 py-2 text-sm font-semibold text-white transition-colors whitespace-nowrap">
              Co.nnect Login
            </a>
          </nav>
        </div>
      ...
      <section className="px-4 sm:px-6 pt-16 sm:pt-24 pb-12 sm:pb-16">
        <div className="mx-auto w-full max-w-[800px] text-center space-y-6">
          <div className="inline-flex items-center gap-2 rounded-full bg-blue-50 border border-blue-100 px-4 py-1.5 text-sm font-medium text-[#2A3CDE]">
            Amazon & Wayfair Vendor Central (1P) Co-Op Partnership
          </div>
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-[#000000] leading-tight">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {VALUE_PROPS.map(({ headline, body }) => (
              <div key={headline} className="rounded-xl border border-slate-100 bg-[#F8FAFC] p-6 space-y-2">
                <h3 className="font-semibold text-[#000000] text-base">{headline}</h3>
      ...
      <section className="px-4 sm:px-6 py-14 sm:py-20">
        <div className="mx-auto w-full max-w-[1100px]">
          <div className="text-center mb-10 sm:mb-14 space-y-2">
            <p className="text-xs uppercase tracking-widest text-[#2A3CDE] font-semibold">How It Works</p>
            <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-[#000000]">Our 1P Co-Op Model</h2>
            ...
            {HOW_IT_WORKS.map(({ step, icon: Icon, title, bullets }) => (
              <div key={step} className="rounded-xl border border-slate-100 bg-white p-6 sm:p-8 space-y-4 shadow-sm">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 flex h-10 w-10 items-center justify-center rounded-lg bg-blue-50 text-[#2A3CDE]">
                    <Icon className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="text-xs font-semibold text-[#2A3CDE] mb-0.5">Step {step}</p>
                    <h3 className="font-semibold text-[#000000] text-base leading-snug">{title}</h3>
                  </div>
                </div>
                <ul className="space-y-2 pl-1">
                  {bullets.map((b) => (
                    <li key={b} className="flex items-start gap-2 text-sm text-slate-600">
                      <CheckCircle2 className="h-4 w-4 text-[#2A3CDE] mt-0.5 shrink-0" />
                      <span>{b}</span>
                    </li>
                  ))}
                </ul>
      ...
      {/* CTA */}
      <section className="px-4 sm:px-6 py-14 bg-[#2A3CDE] text-white">
        <div className="mx-auto w-full max-w-[800px] text-center space-y-5">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <a href={`${CONNECT}/analyzer`} className="inline-flex items-center rounded-lg bg-white px-6 py-3 text-sm font-semibold text-[#000000] hover:bg-blue-50 transition-colors">
              Upload ASINs into Wholesale Analyzer
            </a>
      ...
      <footer className="border-t bg-white">
        <div className="mx-auto w-full max-w-[1100px] px-4 sm:px-6 py-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          <div className="text-xs text-slate-500 space-y-1">
            ...
            <p><a href="mailto:info@nice-supply.com" className="text-[#2A3CDE] hover:underline">info@nice-supply.com</a> · Mon-Fri, 9 AM-5 PM</p>
          </div>
          <div className="flex items-center gap-4 text-sm">
            <a href={`${CONNECT}/calculator`} className="text-slate-500 hover:text-[#2A3CDE] transition-colors">Wholesale Calculator</a>
            <a href={`${CONNECT}/analyzer`} className="text-slate-500 hover:text-[#2A3CDE] transition-colors">ASIN Analyzer</a>
            <a href={`${CONNECT}/register`} className="text-slate-500 hover:text-[#2A3CDE] transition-colors">Apply as Partner</a>
            <a href={`${CONNECT}/login`} className="text-[#2A3CDE] font-medium hover:underline">Co.nnect Login →</a>
          </div>
        </div>
      </footer>
    </div>
